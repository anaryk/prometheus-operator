...
func (cg *ConfigGenerator) generatePodMonitorConfig(
	...
	cfg = append(cfg, cg.generateK8SSDConfig(m.Spec.NamespaceSelector, m.Namespace, apiserverConfig, s, kubernetesSDRolePod, attachMetaConfig, nil, nil))
...
func (cg *ConfigGenerator) generateProbeConfig(
	...
	case m.Spec.Targets.Ingress != nil:
		...
		cfg = append(cfg, cg.generateK8SSDConfig(m.Spec.Targets.Ingress.NamespaceSelector, m.Namespace, apiserverConfig, s, kubernetesSDRoleIngress, nil, nil, nil))
...
func (cg *ConfigGenerator) generateServiceMonitorConfig(
	...
	cfg = append(cfg, cg.generateK8SSDConfig(m.Spec.NamespaceSelector, m.Namespace, apiserverConfig, s, role, attachMetaConfig, &m.Spec.Selector, &m.Spec.SelectorMechanism))
...
func (cg *ConfigGenerator) generateK8SSDConfig(
	...
	if ptr.Deref(selectorMechanism, monitoringv1.SelectorMechanismRelabel) == monitoringv1.SelectorMechanismRole {
		k8sSDConfig = generateRoleSelectorConfig(role, selector, k8sSDConfig, cg)
	}
...
func generateRoleSelectorConfig(role string, selector *metav1.LabelSelector, k8sSDConfig yaml.MapSlice, cg *ConfigGenerator) yaml.MapSlice {
	...
	if len(selector.MatchLabels) > 0 {
		labels := []string{}
		for _, k := range util.SortedKeys(selector.MatchLabels) {
			labels = append(labels, sanitizeLabelName(k)+"="+selector.MatchLabels[k])
		}
		yml = append(yml, yaml.MapItem{Key: "label", Value: strings.Join(labels, ",")})
	...
	if len(selector.MatchExpressions) > 0 {
		...
		for _, exp := range selector.MatchExpressions {
			switch exp.Operator {
			case metav1.LabelSelectorOpIn:
				expressions = append(expressions, fmt.Sprintf("%s in (%s)", sanitizeLabelName(exp.Key), strings.Join(exp.Values, ", ")))
			case metav1.LabelSelectorOpNotIn:
				expressions = append(expressions, fmt.Sprintf("%s notin (%s)", sanitizeLabelName(exp.Key), strings.Join(exp.Values, ", ")))
			case metav1.LabelSelectorOpExists:
				expressions = append(expressions, fmt.Sprintf("%s exists", sanitizeLabelName(exp.Key)))
			case metav1.LabelSelectorOpDoesNotExist:
				expressions = append(expressions, fmt.Sprintf("%s doesnotexist", sanitizeLabelName(exp.Key)))
			}
		}
...
func (cg *ConfigGenerator) generateAlertmanagerConfig(alerting *monitoringv1.AlertingSpec, apiserverConfig *monitoringv1.APIServerConfig, store assets.StoreGetter) []yaml.MapSlice {
	...
	for i, am := range alerting.Alertmanagers {
		...
		cfg = append(cfg, cg.generateK8SSDConfig(monitoringv1.NamespaceSelector{}, ns, apiserverConfig, store, cg.endpointRoleFlavor(), nil, nil, nil))
