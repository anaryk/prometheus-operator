...
func TestNamespaceSetCorrectly(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		c := cg.generateK8SSDConfig(tc.ServiceMonitor.Spec.NamespaceSelector, tc.ServiceMonitor.Namespace, nil, assets.NewTestStoreBuilder().ForNamespace(tc.ServiceMonitor.Namespace), kubernetesSDRoleEndpoint, attachMetaConfig, nil, nil)
...
func TestNamespaceSetCorrectlyForPodMonitor(t *testing.T) {
	...
	c := cg.generateK8SSDConfig(pm.Spec.NamespaceSelector, pm.Namespace, nil, assets.NewTestStoreBuilder().ForNamespace(pm.Namespace), kubernetesSDRolePod, attachMetadataConfig, nil, nil)
	...
	cfg, err := cg.GenerateServerConfiguration(
		...
		map[string]*monitoringv1.ServiceMonitor{
			"test": {
				...
				Spec: monitoringv1.ServiceMonitorSpec{
					SelectorMechanism: monitoringv1.SelectorMechanismRelabel,
					Selector: metav1.LabelSelector{
						MatchLabels: map[string]string{
							"foo": "bar",
						},
